"""Preprocessing utilities for three-day forecast data."""

import logging

import pandas as pd

from app.ml.three_day.loader import load_dataset
...
def preprocess_dataset(dataframe: pd.DataFrame | None = None) -> pd.DataFrame:
    """Clean and feature-engineer three-day forecast data.
    ...
    Args:
        dataframe: Optional raw three-day forecast dataframe to preprocess.
    ...
    logger.info(
        "Removed %d duplicate rows from three-day forecast data",
        original_row_count - len(dataframe),
    )
    ...
    logger.info("Preprocessed %d rows of three-day forecast data", len(dataframe))
    return dataframe
